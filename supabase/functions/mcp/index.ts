...
import { defineTool as defineTool14, ToolError as ToolError16 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z14 } from "npm:zod@^3.25.76";

// src/lib/mcp/datasheets.ts
import { ToolError as ToolError15 } from "npm:@lovable.dev/mcp-js@3.0.1";
var DATASHEET_BUCKET = "product-images";
var DATASHEET_FOLDER = "datasheets";
var DATASHEET_MAX_BYTES = 10 * 1024 * 1024;
var FETCH_TIMEOUT_MS = 2e4;
var DATASHEET_PATH_PATTERN = /^datasheets\/(?!.*\.\.)[A-Za-z0-9._\-/]+\.pdf$/;
...
async function downloadPdf(url, notFoundMessage) {
  ...
  if (notFoundMessage && (res.status === 400 || res.status === 404)) throw new ToolError15(notFoundMessage);
  if (!res.ok || !res.body) throw new ToolError15(`Could not download the PDF (HTTP ${res.status})`);
  const declared = Number(res.headers.get("content-length") ?? 0);
  if (declared > DATASHEET_MAX_BYTES) throw new ToolError15("PDF is larger than 10 MB");
  const chunks = [];
  let total = 0;
...
function safePdfName(fileName) {
  const base = fileName.trim().replace(/\.pdf$/i, "").toLowerCase().replace(/[^a-z0-9.-]+/g, "-").replace(/-+/g, "-").replace(/^[-.]+|[-.]+$/g, "").slice(0, 120);
...
var set_product_datasheet_default = defineTool14({
  ...
  description: "Admin only. Attach a PDF datasheet to one product (id) or up to 100 products at once (product_ids), replacing any existing one, or remove it. Provide exactly one source: storage_path (a file already uploaded via create_datasheet_upload; preferred for local files), pdf_url (an https link to a PDF; it is downloaded and stored on the site), pdf_base64 (the PDF file content), or remove=true. Max 10 MB. A previously attached file is deleted only once no product uses it any more.",
  inputSchema: {
    id: z14.string().uuid().optional().describe("Product id (use this or product_ids)."),
    product_ids: z14.array(z14.string().uuid()).min(1).max(100).optional().describe("Up to 100 product ids that all get the same datasheet (use this or id)."),
    storage_path: z14.string().regex(DATASHEET_PATH_PATTERN, "storage_path must be a .pdf path inside datasheets/").optional().describe("storage_path returned by create_datasheet_upload. The file must already be uploaded."),
    ...
    remove: z14.boolean().optional().describe("true to remove the datasheet.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  handler: async ({ id, product_ids, storage_path, pdf_url, pdf_base64, file_name, remove }, ctx) => {
    if (id === void 0 === (product_ids === void 0)) throw new ToolError16("Provide exactly one of id or product_ids");
    const sources = [storage_path, pdf_url, pdf_base64, remove ? true : void 0].filter((v) => v !== void 0);
    if (sources.length !== 1) throw new ToolError16("Provide exactly one of storage_path, pdf_url, pdf_base64, or remove=true");
    ...
    if (readError) throw new ToolError16(readError.message);
    const found = current ?? [];
    const missing = ids.filter((pid) => !found.some((row) => row.id === pid));
    if (missing.length) throw new ToolError16(`No product found with id ${missing.join(", ")}`);
    ...
    if (storage_path) {
      ...
      if (!isPdf(bytes)) throw new ToolError16("The file at storage_path is not a PDF");
      ...
      if (bytes.length > DATASHEET_MAX_BYTES) throw new ToolError16("PDF is larger than 10 MB");
      if (!isPdf(bytes)) throw new ToolError16("The file is not a PDF");
      ...
      if (uploadError) throw new ToolError16(`Upload failed: ${uploadError.message}`);
    ...
    const { data, error } = await supabase.from("products").update({ datasheet_url: datasheetUrl, datasheet_name: datasheetName }).in("id", ids).select(PRODUCT_COLUMNS);
    const updated = data ?? [];
    if (error || updated.length !== ids.length) {
      if (uploadedPath) await bucket.remove([uploadedPath]);
      throw new ToolError16(error?.message ?? `Updated ${updated.length} of ${ids.length} products`);
    }
    ...
    for (const [path, url] of previous) {
      const { count, error: countError } = await supabase.from("products").select("id", { count: "exact", head: true }).eq("datasheet_url", url);
      if (!countError && count === 0) await bucket.remove([path]);
    }
    const products = updated.map(toProduct);
    const summary = remove ? `Datasheet removed from ${products.length} product(s).` : `Datasheet attached to ${products.length} product(s): ${datasheetName}.`;
    return id !== void 0 ? jsonResult(summary, { product: products[0] }) : jsonResult(summary, { count: products.length, products });
  }
});

// src/lib/mcp/tools/create-datasheet-upload.ts
import { defineTool as defineTool15, ToolError as ToolError17 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z15 } from "npm:zod@^3.25.76";
var create_datasheet_upload_default = defineTool15({
  name: "create_datasheet_upload",
  title: "Create datasheet upload URL",
  description: "Admin only. Reserve a storage path for a PDF datasheet and return a signed upload URL, so a local file can be sent straight to storage (e.g. with curl) instead of through a tool call. Upload the file with an HTTP PUT to upload_url (valid for 2 hours), then attach it with set_product_datasheet using storage_path (optionally product_ids for many products at once).",
  inputSchema: {
    file_name: z15.string().trim().min(1).max(200).regex(/\.pdf$/i, "file_name must end in .pdf").describe("Name of the PDF being uploaded, e.g. 'Indoor-Fiber-Patch-Cord.pdf'.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  handler: async ({ file_name }, ctx) => {
    ...
    if (error || !data) throw new ToolError17(`Could not create upload URL: ${error?.message ?? "no data"}`);
...
import { defineTool as defineTool16, ToolError as ToolError18 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z16 } from "npm:zod@^3.25.76";
var create_category_default = defineTool16({
  ...
  inputSchema: {
    ...
    key: z16.string().trim().min(1).optional().describe("Unique key (a-z, 0-9, dashes). Defaults to a slug of name_en."),
    sort: z16.number().int().min(0).optional().describe("Display order. Defaults to last.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  handler: async ({ name_ar, name_en, key, sort }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
    const finalKey = slugify(key ?? name_en);
    if (!finalKey) throw new ToolError18("Could not derive a key from name_en; provide key explicitly");
    let finalSort = sort;
    if (finalSort === void 0) {
      const { count, error: error2 } = await supabase.from("categories").select("id", { count: "exact", head: true });
      if (error2) throw new ToolError18(error2.message);
      finalSort = count ?? 0;
    }
...
import { defineTool as defineTool17, ToolError as ToolError19 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z17 } from "npm:zod@^3.25.76";
var update_category_default = defineTool17({
  name: "update_category",
  title: "Update category",
  description: "Admin only. Update a product category. Only the fields provided are changed.",
  inputSchema: {
    id: z17.string().uuid().describe("Category id."),
    name_ar: z17.string().trim().min(1).optional().describe("Arabic name."),
    name_en: z17.string().trim().min(1).optional().describe("English name."),
    key: z17.string().trim().min(1).optional().describe("Unique key (a-z, 0-9, dashes)."),
    sort: z17.number().int().min(0).optional().describe("Display order.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async ({ id, key, ...fields }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
    const patch = compact({ ...fields, key: key === void 0 ? void 0 : slugify(key) });
    if (patch.key === "") throw new ToolError19("key must contain letters or digits");
    assertHasChanges(patch);
...
import { defineTool as defineTool18, ToolError as ToolError20 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z18 } from "npm:zod@^3.25.76";
var create_subcategory_default = defineTool18({
  name: "create_subcategory",
  title: "Create subcategory",
  description: "Admin only. Create a subcategory under an existing category.",
  inputSchema: {
    category_id: z18.string().uuid().describe("Parent category id."),
    name_ar: z18.string().trim().min(1).describe("Arabic name."),
    name_en: z18.string().trim().min(1).describe("English name.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  handler: async (input, ctx) => {
    const { supabase } = await requireAdmin(ctx);
...
import { defineTool as defineTool19, ToolError as ToolError21 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z19 } from "npm:zod@^3.25.76";
var update_subcategory_default = defineTool19({
  ...
  inputSchema: {
    id: z19.string().uuid().describe("Subcategory id."),
    category_id: z19.string().uuid().optional().describe("New parent category id."),
    name_ar: z19.string().trim().min(1).optional().describe("Arabic name."),
    name_en: z19.string().trim().min(1).optional().describe("English name.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async ({ id, ...fields }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
...
import { defineTool as defineTool20, ToolError as ToolError22 } from "npm:@lovable.dev/mcp-js@3.0.1";
...
// src/lib/mcp/tools/create-brand.ts
var create_brand_default = defineTool20({
  name: "create_brand",
  title: "Create brand",
  description: "Admin only. Create a brand.",
  inputSchema: {
    name: z20.string().trim().min(1).describe("Brand name."),
    slug: z20.string().trim().min(1).optional().describe("Unique URL slug. Defaults to a slug of name."),
    description: z20.string().trim().nullable().optional().describe("Description, or null."),
    logo_url: z20.string().url().nullable().optional().describe("Logo image URL, or null."),
    is_active: z20.boolean().optional().describe("Show on the store. Defaults to true."),
    sort: z20.number().int().min(0).optional().describe("Display order. Defaults to 0.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  handler: async ({ name, slug, description, logo_url, is_active, sort }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
    const finalSlug = slugify(slug ?? name);
    if (!finalSlug) throw new ToolError22("Could not derive a slug from name; provide slug explicitly");
...
import { defineTool as defineTool21, ToolError as ToolError23 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z21 } from "npm:zod@^3.25.76";
var update_brand_default = defineTool21({
  name: "update_brand",
  title: "Update brand",
  ...
  inputSchema: {
    id: z21.string().uuid().describe("Brand id."),
    name: z21.string().trim().min(1).optional().describe("Brand name."),
    slug: z21.string().trim().min(1).optional().describe("Unique URL slug."),
    description: z21.string().trim().nullable().optional().describe("Description, or null to clear."),
    logo_url: z21.string().url().nullable().optional().describe("Logo image URL, or null to clear."),
    is_active: z21.boolean().optional().describe("Show (true) or hide (false) on the store."),
    sort: z21.number().int().min(0).optional().describe("Display order.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async ({ id, slug, ...fields }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
    const patch = compact({ ...fields, slug: slug === void 0 ? void 0 : slugify(slug) });
    if (patch.slug === "") throw new ToolError23("slug must contain letters or digits");
    assertHasChanges(patch);
...
import { defineTool as defineTool22, ToolError as ToolError24 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z22 } from "npm:zod@^3.25.76";
var ORDER_STATUSES = ["pending", "processing", "shipped", "delivered", "canceled"];
var update_order_status_default = defineTool22({
  name: "update_order_status",
  title: "Update order status",
  description: "Admin only. Change an order's status.",
  inputSchema: {
    id: z22.string().uuid().describe("Order id."),
    status: z22.enum(ORDER_STATUSES).describe("New status.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
...
import { defineTool as defineTool23, ToolError as ToolError25 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z23 } from "npm:zod@^3.25.76";
var QUOTE_STATUSES = ["new", "contacted", "closed"];
var update_quote_request_status_default = defineTool23({
  name: "update_quote_request_status",
  title: "Update quote request status",
  description: "Admin only. Change a quote request's status.",
  inputSchema: {
    id: z23.string().uuid().describe("Quote request id."),
    status: z23.enum(QUOTE_STATUSES).describe("New status.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async ({ id, status }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
...
import { defineTool as defineTool24, ToolError as ToolError26 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z25 } from "npm:zod@^3.25.76";

// src/lib/mcp/content.ts
import { z as z24 } from "npm:zod@^3.25.76";
var text2 = z24.string().trim();
...
var blogFields = {
  ...
  cover_url: z24.string().url().nullable().optional().describe("Cover image URL, or null."),
  is_featured: z24.boolean().optional().describe("Feature on the home page."),
  featured_sort: z24.number().int().min(0).optional().describe("Order among featured posts.")
};
...
var projectFields = {
  ...
  cover_url: z24.string().url().nullable().optional().describe("Cover image URL, or null."),
  gallery: z24.array(z24.string().url()).optional().describe("Gallery image URLs."),
  client: text2.nullable().optional().describe("Client name, or null."),
  location: text2.nullable().optional().describe("Location, or null."),
  completed_at: z24.string().date().nullable().optional().describe("Completion date (YYYY-MM-DD), or null."),
  sort: z24.number().int().min(0).optional().describe("Display order.")
};
...
// src/lib/mcp/tools/create-blog-post.ts
var create_blog_post_default = defineTool24({
  ...
  inputSchema: {
    slug: slugField,
    ...blogFields,
    status: z25.enum(["draft", "published"]).optional().describe("Defaults to draft.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
...
import { defineTool as defineTool25, ToolError as ToolError27 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z26 } from "npm:zod@^3.25.76";
var update_blog_post_default = defineTool25({
  ...
  inputSchema: {
    id: z26.string().uuid().describe("Blog post id."),
    ...
    status: z26.enum(["draft", "published"]).optional().describe("draft or published.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async ({ id, status, ...fields }, ctx) => {
    ...
    if (status !== void 0) {
      ...
      if (error2) throw new ToolError27(error2.message);
...
import { defineTool as defineTool26, ToolError as ToolError28 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z27 } from "npm:zod@^3.25.76";
var create_project_default = defineTool26({
  name: "create_project",
  ...
  inputSchema: {
    slug: slugField,
    ...projectFields,
    is_published: z27.boolean().optional().describe("Show on the website. Defaults to false.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  handler: async (input, ctx) => {
    const { supabase, userId } = await requireAdmin(ctx);
...
import { defineTool as defineTool27, ToolError as ToolError29 } from "npm:@lovable.dev/mcp-js@3.0.1";
import { z as z28 } from "npm:zod@^3.25.76";
var update_project_default = defineTool27({
  name: "update_project",
  title: "Update project",
  description: "Admin only. Update a portfolio project. Only the fields provided are changed.",
  inputSchema: {
    id: z28.string().uuid().describe("Project id."),
    slug: slugField.optional(),
    ...projectFields,
    title_ar: projectFields.title_ar.optional(),
    title_en: projectFields.title_en.optional(),
    is_published: z28.boolean().optional().describe("Show (true) or hide (false) on the website.")
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async ({ id, ...fields }, ctx) => {
    const { supabase } = await requireAdmin(ctx);
...
var mcp_default = defineMcp({
  ...
  tools: [
    ...
    create_datasheet_upload_default,
